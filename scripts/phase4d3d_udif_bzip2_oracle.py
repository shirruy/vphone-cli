...
def deterministic_payload(size: int) -> bytes:
    ...
    state = 0x51A7C0DE
    for i in range(size):
        state = (state * 1103515245 + 12345 + i) & 0xFFFFFFFF
        if (i // (256 * 1024)) % 4 == 0:
            out[i] = (i // SECTOR) & 0xFF
        else:
            out[i] = (state ^ (state >> 11) ^ (i * 29)) & 0xFF
...
def make_udif(path: Path, raw: bytes, payload_type=BZIP2):
    ...
    for offset in range(0, len(raw), chunk_size):
        ...
        if payload_type == BZIP2:
            compressed = bz2.compress(chunk, compresslevel=9)
        else:
            compressed = chunk[: min(len(chunk), 4096)]
...
def main() -> int:
    ...
    dmg = workdir / "bzip2.dmg"
    restored = workdir / "restored.raw"
    corrupt = workdir / "corrupt-bzip2.dmg"
